...
import entrainement_list  # Importation des données d'entraînement
...
# Fonction de modification de la phrase pour remplacer certains caractères par des tokens
...
# Création du vocabulaire à partir des données d'entraînement
...
# Fonction d'entraînement du modèle LSTM
...
# Échantillonnage parmi les k meilleurs résultats pour la génération de réponses
def top_k_sampling(output, k=5):
    top_k = torch.topk(output, k).indices.squeeze(0).tolist()
    return random.choice(top_k)

# Fonction pour générer une réponse à partir d'un modèle entraîné
def generate_response(model, input_text, word_to_ix, ix_to_word, accelerator, max_length=20, k=5):
    ...
    # Remplacement des tokens par les caractères originaux pour l'affichage final
    for char, token in char_to_token:
        generated_text = generated_text.replace(token, char)
    
    return generated_text

# Classe définissant le modèle LSTM utilisé pour l'apprentissage
...
# Code principal exécutant l'ensemble du processus d'entraînement et d'interaction utilisateur
def main():
    # Initialisation de l'accélérateur pour gérer les opérations sur GPU/CPU efficacement
    accelerator = Accelerator()
    
    # Chargement et prétraitement des données d'entraînement
    entrainement = preprocess_data()
    
    # Création du vocabulaire à partir des données prétraitées 
   ...
   # Entraînement du modèle avec les données prétraitées 
   train_model(model ,criterion ,optimizer ,num_epochs ,entrainement ,word_to_ix ,accelerator) 

   print("AI trained and ready!")

   # Dictionnaire associant les commandes utilisateur aux actions correspondantes 
   command_actions={
       '/q':lambda :'quit',
       '/quit':lambda :'quit',
